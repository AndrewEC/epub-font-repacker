...
class Opf:
    ...
    def find_path_to_opf_file(self, temp_path: Path) -> Path:
        ...
        This searches for the location of the opf file by reading in the standard container.xml
        file, looking for the list of files specified within the container->rootfiles->rootfile
        node, and searching for a file defined with a media-type of application/oebps-package+xml.

        :param temp_path: The path to the folder where the epub contents were extracted to.
        ...
        """
        try:
            container_content = self._read_container_file(temp_path)
            relative_opf_location = self._find_relative_opf_file_location(container_content)
    ...
    def add_manifest_entry_to_opf_file(self, temp_path: Path, opf_manifest_item: str):
        """
        Finds the location of the OPF file extracted from the input epub file and inserts
        the specified manifest entry at the end of the manifest just before the closing </manifest>
        tag.
        ...
        :raises OpfLocationException: Raised if the OPF file cannot be found.
        """
        opf_file_path = self.find_path_to_opf_file(temp_path)
    ...
    def _read_container_file(self, temp_path: Path) -> str:
